...
namespace ft
{
	...
	{
		public:
			typedef Alloc												allocator_type_2;
			typedef Tree<ft::pair<const Key, T> >						node;
		...
		public:
			...
			: m_alloc(alloc), m_comp(comp), m_root(NULL), m_size(0)
			{
				m_end = m_alloc.allocate(1);
				m_alloc.construct(m_end, Tree<value_type>(NULL));
			}
			map(const map &src)
			:m_alloc(src.m_alloc), m_comp(src.m_comp), m_root(NULL), m_size(src.m_size)
			{
		...
		private:
			...
			Tree<value_type> *create_tree(const value_type &val)
			{
				...
				Tree<value_type> *node = m_alloc.allocate(1);
				m_alloc.construct(node, Tree<value_type>(p));
			...
			Tree<typename map<Key, T, Compare, Alloc>::value_type> *clone_tree(Tree <value_type> *node, Tree <value_type> *parent)
			{
				if (!node)
					return (NULL);
