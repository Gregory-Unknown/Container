...
namespace ft {
	...
	template<class Key, class V, class P = ft::pair<const Key, V>,  class Alloc = std::allocator<P>, class Compare = std::less<Key> >
	class tree
	{
	public:
		typedef node<P>												node;
		...
		void printTreeHelper(node *root, int space)
	{
		...
		if(root != NULL)
		{
			...
			if(root == this->root)
				// std::cout << "root  " << root->color << "  "<< root->_pair.getFirst() << std::endl;
				std::cout << "root  " << root->color << "  "<< root->data.getFirst() << std::endl;
			else
				// std::cout << root->color << "  " << root->_pair.getFirst() << std::endl;
				std::cout << root->color << "  " << root->data.getFirst() << std::endl;
			std::cout << std::endl;
	...
	void test_show_tree()
	{
		...
		node *insertElem(node *elem, node *start) {
			...
			} else if (elem->data.getFirst() == start->data.getFirst())
				return (NULL);
			else {
				if (start->right)
		...
		void deleteFix(node* x, node * parent) {
			...
			void displayTree(node *root, std::string indent, bool last) {
				if (root != NULL) {
					...
					std::cout << sColor << root->data.getFirst() << RESETC << std::endl;
